...
export function emptyInventory(bot: Bot, chestLocations: Vec3[], itemFilter: ItemFilter, cb: Callback): void
{
    ...
    const tryNextChest = () => {
        ...
        tryEmptyInventory(bot, chest, itemFilter, (hasRemaining, err) => {
            if (err)
            {
                cb(err);
                return;
...
function tryEmptyInventory(bot: Bot, chestLocation: Vec3, itemFilter: ItemFilter, cb: (hasRemaining: boolean, err?: Error) => void): void
{
...
function gotoChest(bot: Bot, location: Vec3, cb: Callback)
{
    // @ts-ignore
...
function placeItems(bot: Bot, chestPos: Vec3, itemFilter: ItemFilter, cb: (hasRemaining: boolean, err?: Error) => void): void
{
    const chestBlock = bot.blockAt(chestPos);
    if (!chestBlock)
    {
        cb(true, error('UnloadedChunk', 'Chest is in an unloaded chunk!'));
        return;
    }
    ...
    try
    {
        ...
        chest.once('open', () => {
            const tryDepositItem = (item: Item, cb: Callback) => {
